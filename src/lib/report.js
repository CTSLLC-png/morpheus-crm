...
export async function generateProgressReportPDF({ participant, calls, eligibility }) {
  ...
  doc.text(`CRM  ·  Certified Training Standards  ·  Albany, NY  ·  ${SITE_HOST}`, lx + 35, 52)
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.setFontSize(6.5)
    doc.setTextColor(...GRAY)
    doc.text(`MORPHEUS CRM  ·  Certified Training Standards  ·  Albany, NY  ·  ${SITE_HOST}`, MAR, PH - 30)
